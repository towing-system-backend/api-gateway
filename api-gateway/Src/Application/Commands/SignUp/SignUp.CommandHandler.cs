...
{
    public class SignUpCommandHandler(IdService<string> idService, IPasswordService passwordService, ICryptoService cryptoService, IEmailService<EmailInfo> emailService, IAccountRepository accountRepository) : IService<SignUpCommand, SignUpResponse>
    {
        ...
        public async Task<Result<SignUpResponse>> Execute(SignUpCommand command)
        {
            ...
            await _accountRepository.Save(new Account(id, command.Email, hashedPassword, DateTime.UtcNow.AddDays(1)));
